...
#### Services (API layer)
- Shared HTTP client: `src/services/http.ts`
  - Reads `VITE_API_BASE_URL`
  - Provides `http<T>` for typed requests
- Auth service: `src/services/auth.ts` (login/logout placeholders)
- Stats service: `src/services/stats.ts` (summary metrics placeholder)
- Content service: `src/services/content.ts` (landing feed placeholder)

To add a new service:
1. Create a module under `src/services/<domain>.ts`
2. Define TypeScript interfaces for request/response shapes
3. Export functions that call `http<T>(path, { method, body, query })`
4. Keep functions small and typed; do not access UI directly
...
export interface Foo { id: string; title: string }
export async function listFoos(): Promise<Foo[]> {
  return http('/api/foos')
}
```
