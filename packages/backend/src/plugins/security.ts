...
export async function generateSigner() {
	const decodedPrivateKey = Buffer.from(JWT_PRIVATE_KEY_BASE64.trim(), 'base64').toString();
...
export async function generateVerifier() {
	const decodedPublicKey = Buffer.from(JWT_PUBLIC_KEY_BASE64.trim(), 'base64').toString();
	const publicCryptoKey = await importSPKI(decodedPublicKey, JWT_ALGORITHM);
